...
The [yardstick](https://yardstick.tidymodels.org/) package has functions for computing both of these measures called `roc_auc()` and `accuracy()`.
...
```{r, echo = FALSE}
knitr::include_graphics("./jpg/resampling.jpg")
```
...
```{r, label = "CV", echo = FALSE, cache = TRUE}
set.seed(345)
...
rf_fit_rs <- 
  rf_wf %>% 
  fit_resamples(folds)
collect_metrics(rf_fit_rs, summarize = FALSE, type = "wide")[, -1] |> 
  kable()
```

```{r, echo = FALSE}
T3 <- collect_metrics(rf_fit_rs)
auc <- T3$mean[3]
acc <- T3$mean[1]
```

From these resampling statistics, the final estimate of performance for this random forest model would be `r auc` for the area under the ROC curve and `r acc` for accuracy.
