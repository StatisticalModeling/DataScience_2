...
knitr::write_bib(c("parsnip", "tidymodels", "recipes", "rsample", "StepReg", "tune"), file = "packages.bib")
```
...
How to perform feature selection with tidymodels:
  ...
  1. Use Regularization: Fit a regularized model like Lasso using `parsnip` written by @R-parsnip, and the regularization process will perform feature selection by shrinking some coefficients to zero. 
  
  2. Manual Selection: You can manually select features by defining a recipe with the chosen predictors using `recipes` written by @R-recipes and then train the model with that specific recipe. 
  
  3. Use External Packages: For true forward selection, you would use other R packages that specifically offer stepwise regression, such as the `StepReg` written by @R-StepReg and available on CRAN. 
  
The **tidymodels** philosophy

The `tidymodels` framework written by @R-tidymodels is designed around a more consistent and robust approach to modeling than traditional stepwise procedures. Key aspects of this philosophy that lead away from forward selection include:
...
* Engine: Specify the `glmnet` engine, which is highly efficient for fitting many values of the `penalty` ($\lambda$) simultaneously.

* Tuning: Use `tune_grid()` from the `tune` package written by @R-tune to find the optimal value of the `penalty` hyperparameter.

* Workflow: The entire process is integrated smoothly into a tidymodels workflow, from preprocessing with `recipes` [@R-recipes] to resampling with `rsample` [@R-rsample] and tuning with `tune` [@R-tune]. 
...
The goal of feature engineering is to transform predictor values that lead to models that better uncover predictor-response relationships.  Feature engineering is a preprocessing of the data that allows one to build better features for modeling.  One can use a `recipe` from the [recipes](https://recipes.tidymodels.org/) package which permits [dplyr](https://dplyr.tidyverse.org/)-like pipeable sequences to reformat data from modeling.  Note that all preprocessing and feature engineering steps use ***only***Boston the training data. In this example, we use a Box-Cox transformation to rescale the numeric predictors to be more similar to a normal distribution as well as the normalizing the numeric predictors to a have a mean of 0 and a standard deviation of 1.       
...
The `recipe()` defines the preprocessing; but, the various steps are not applied to the training data until the recipe is fit to the training set with `prep()` and finally applied to the training data with the `bake()` function.  Note that `prep()` for a recipe is like `fit()` for a model and `bake()` for a recipe is similar to  `predict()` for a model.
...
First we will ignore the previously normalized and transformed variables and use the raw data.  Then, we will go back and use the previous `boston_recipe` with the transformations and normalizations to see how the models compare.
...
```{r}
relocate(augment(lm_fit, new_data = boston_testing), 
         medv, .after =.pred)
...
augment(boston_fit, new_data = boston_testing) |> 
  metrics(truth = medv, estimate = .pred) -> T3
T3
# Note: this is RMSE_Training
augment(boston_fit, new_data = boston_training) |> 
  metrics(truth = medv, estimate = .pred) -> T4
T4
```
By normalizing and appling transformations to the data the $\text{RMSE}_\text{test} = `r T3$.estimate[1]`$ compared to the untransformed and unnormalized $\text{RMSE}_\text{test} = `r T1$.estimate[1]`$.
...
boston_training <- training(boston_split)
boston_testing <- testing(boston_split)
# This is untransformed and not normalized
lm_fit <- lm_spec |> 
  fit(medv ~ ., data = boston_training)
augment(lm_fit, new_data = boston_testing)
augment(lm_fit, new_data = boston_testing) |> 
  rmse(truth = medv, estimate = .pred) -> T5
T5
augment(lm_fit, new_data = boston_training) |> 
  rmse(truth = medv, estimate = .pred) -> T6
T6
boston_recipe <- 
  recipe(formula = medv ~ ., data = boston_training) |> 
  step_BoxCox(all_numeric_predictors()) |> 
...
Remember this is with untransformed and unnormalized data so the (`set.seed(3)`) $\text{RMSE}_\text{test} = `r T5$.estimate[1]`$ must be compared with (`set.seed(3178)`) $\text{RMSE}_\text{test} = `r T1$.estimate[1]`$.  Changing the seed had a considerable impact on the $\text{RMSE}_\text{test}$!  To reduce the variability from the arbitrary splitting of data into training and testing data, we will create many training and testing sets and average the $\text{RMSE}$ using k-fold cross-validation.
...
#| cache: true
set.seed(23)
boston_folds <- vfold_cv(boston_training, v = 10, repeats = 5)
boston_folds
...
```{r}
ridge_spec <- linear_reg(mixture = 0, penalty = 0.001) |> 
  set_mode("regression") |> 
  set_engine("glmnet")
...
```
So how to we find the "best" `penalty` (`penalty` in tidymodels $\equiv \lambda$, note: $\lambda = \text{penalty} = 0 \text{ and } \alpha = \text{mixture} = 0$ is normal regression.  However, `glmnet` uses an iterative coordinate descent algorithm to find coefficient estimates by solving a penalized maximum likelihood problem.  The iterative process introduces minor numerical differences when compared to a direct calculation such as the one used with `lm()`)?  We can use tuning to find the best hyperparameter.  For now, we will only consider a grid search with evenly spaced parameter values to optimize our model.
...
We need values of the `penalty`.  Looking at the graph from `autoplot()`, we select values from 0.001 to 1000 using `grid_regular()` from the `dials` package which will create a grid of evenly spaced values.

```{r}
penalty_grid <- grid_regular(penalty(range = c(-3, 3)), levels = 20)
penalty_grid
```
...
Start by creating a `workflow_set`.  The `workflowsets` package is used to create a collection of different modeling pipelines, each potentially containing a different model specification and/or preprocessing recipe.

```{r}
library(workflowsets)  # Note: this is part of tidymodels
